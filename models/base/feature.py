...
def extract_feat_vgg_new(img, backbone, feat_ids, bottleneck_ids=None, lids=None):
    feats = []
    feat = img
    final_feat = None
    for lid, module in enumerate(backbone.features):
        ...
        if lid in feat_ids:
            feats.append(feat.clone())
        if lid == len(backbone.features) - 1:
            final_feat = feat.clone()
    return feats, final_feat
...
def extract_feat_res_new(img, backbone, feat_ids, bottleneck_ids, lids):
    r""" Extract intermediate features from ResNet"""
    feats = []
    final_feat = None

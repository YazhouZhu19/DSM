...
class TestDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        if args['dataset'] == 'Cardiac':
            ...
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'augmented/image*'))
    ...
    def __getitem__(self, idx):
        ...
        img = (img - img.mean()) / img.std()  # (B, H, W, C)
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        if args['dataset'] == 'Cardiac':
            ...
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'augmented/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'augmented/label*'))
        elif args['dataset'] == 'CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'augmented/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'augmented/label*'))
    ...
    def __getitem__(self, idx):
        ...
        sup_lbl = lbl_cls[sample[:self.n_shot * self.n_way]][None,]  # n_way * n_shot * H * W
        qry_lbl = lbl_cls[sample[self.n_shot * self.n_way:]]  # n_qry * H * W

        sup_img = img[sample[:self.n_shot * self.n_way]][None,]  # n_way * n_shot * H * W * C
        sup_img = np.transpose(sup_img, (0, 1, 4, 2, 3))  # n_way * n_shot * C * H * W
        # sup_img = np.stack((sup_img, sup_img, sup_img), axis=2)  # stack operation

        qry_img = img[sample[self.n_shot * self.n_way:]]  # n_qry * H * W * C
        qry_img = np.transpose(qry_img, (0, 3, 1, 2))  # n_qry * C * H * W
        ...
        sup_lbl_bg = 1.0 - sup_lbl  # added by myself

        sample = {'support_images': sup_img,
                  'support_fg_labels': sup_lbl,
                  'support_bg_labels': sup_lbl_bg,  # added by myself

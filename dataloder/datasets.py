...
class TestDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        elif args['dataset'] == 'Prostate_Biopsy':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'biopsy_normalized/image*'))
        elif args['dataset'] == 'Prostate_TCIA_PD':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'tcia_pd_normalized/image*'))
        elif args['dataset'] == 'Prostate_TCIA_P3T':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'tcia_p3t_normalized/image*'))
        elif args['dataset'] == 'MM_WHS_MR':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'whs_mr_normalized/image*'))
        elif args['dataset'] == 'MM_WHS_CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'whs_ct_normalized/image*'))
        elif args['dataset'] == 'AMOS_CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'amos_ct_normalized/image*'))
        elif args['dataset'] == 'AMOS_MRI':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'amos_mri_normalized/image*'))
        elif args['dataset'] == 'MSD_CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
        elif args['dataset'] == 'MSD_MRI':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
        elif args['dataset'] == 'MSD_CT_Spleen':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
        elif args['dataset'] == 'MI-PRO':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
        elif args['dataset'] == 'MS-CMR':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
    ...
    def resize_array(self, data):
        n = data.shape[0]
        resized = np.zeros((n, 256, 256))
        for i in range(n):
            resized[i] = cv2.resize(data[i], (256, 256), interpolation=cv2.INTER_LINEAR)
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        if args['dataset'] == 'CARDIAC_bssFP':
            ...
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'biopsy_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'biopsy_normalized/label*'))
        elif args['dataset'] == 'Prostate_TCIA_P3T':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'tcia_p3t_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'tcia_p3t_normalized/label*'))
        elif args['dataset'] == 'Prostate_TCIA_PD':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'tcia_pd_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'tcia_pd_normalized/label*'))
        elif args['dataset'] == 'MM_WHS_MR':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'whs_mr_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'whs_mr_normalized/label*'))
        elif args['dataset'] == 'MM_WHS_CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'whs_ct_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'whs_ct_normalized/label*'))
        elif args['dataset'] == 'AMOS_CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'amos_ct_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'amos_ct_normalized/label*'))
        elif args['dataset'] == 'AMOS_MRI':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'amos_mri_normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'amos_mri_normalized/label*'))
        elif args['dataset'] == 'MSD_CT':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/label*'))
        elif args['dataset'] == 'MSD_MRI':
            self.image_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/image*'))
            self.label_dirs = glob.glob(os.path.join(args['data_dir'], 'normalized/label*'))
        elif args['dataset'] == 'MSD_CT_Spleen':
    ...
    def __getitem__(self, idx):
        ...
        if self.read:
            ...
            gt = self.resize_array(gt)
            # gt = cv2.resize(gt, (256, 256), interpolation=cv2.INTER_LINEAR)

            sprvxl = self.sprvxls[self.sprvxl_dirs[pat_idx]]
            sprvxl = self.resize_array(sprvxl) 
            # sprvxl = sprvxl.transpose(1, 2, 0)
            # sprvxl = cv2.resize(sprvxl, (256, 256), interpolation=cv2.INTER_LINEAR)
            # sprvxl = sprvxl.transpose(2, 0, 1) 
            ...
            sprvxl = sitk.GetArrayFromImage(sitk.ReadImage(self.sprvxl_dirs[pat_idx]))  # label of supervoxel
            sprvxl = cv2.resize(sprvxl, (256, 256), interpolation=cv2.INTER_LINEAR)
